...
def split_data():
    with open(os.path.realpath(__file__).replace("split.py", "data.txt"), 'r', encoding='UTF-8') as file:
        content = file.read()
        lines = content.split(";")
        ...
        for i in tqdm(range(0,len(lines)-1)):
            ...
            for n in range( 0 , len(separate_words)):
                if n >=1:
                    ...
                    req_to_spec = ReqToSpec.get_or_create(speciality_id=spec_id[0], requirement_id=req_id[0])
